...
- **列表展示**：
  ...
  - 页面标题“分类管理”在导航栏中居中展示；左侧返回，右侧保留“新增”入口。
...
- 预设一级分类与自定义一级分类卡片采用同一信息结构：均展示一级分类名称、二级分类摘要、二级分类明细；差异仅体现在状态字段（如“启用”/“已停用·可恢复”）与状态色。
- 一级分类名称左侧展示圆形彩色背景分类图标，图标样式与 Design System 中“分类图标（Lucide）”及 `Component/CategoryItem` 的 `c1` 规范保持一致。

**新增分类抽屉表单（新增）**
 - 触发方式：点击右上角"新增"按钮，从底部弹出抽屉表单。
 - 抽屉内容：
   - 顶部栏：左侧标题“新增分类”，右侧 `x` 关闭按钮。
   - 类型切换：在抽屉内加入与分类管理页一致的 `typeTabsWrap` 组件，水平居中展示，用于单选“支出 / 收入”；`drawerTypeTabs` 的区块色彩、选中背景、字体颜色需与页面 `typeTabsWrap` 完全一致。
   - 一级分类基础信息：名称、类型（支出/收入）、**图标选择**、颜色。
   - 二级分类列表：支持动态新增多条二级分类（每条含名称与**图标选择**）。
   - 一级分类图标展示区：位于一级分类名称输入框下方，仅展示当前一级分类图标项并支持点击选中；不展示图标项关闭叉按钮，不保留“输入图标标签后回车添加”的 `iconAddRow` / 文本区块。
 ...
 - 图标选择规则：
   ...
   - 图标展示与选择交互统一使用 Design System 页 `Component/CategoryItem` 的 `c1` 图标规范：圆形浅色背景 + 彩色 Lucide 图标 + 辅助标签/删除控制。
 - 交互规则：
   - 未填写一级分类名称时不可保存。
   - 二级分类名称可为空；若为空则该条不创建。
   - 同类型下一级分类名称不可重复；同父节点下二级分类名称不可重复。
   - 一级分类图标可点击切换选中状态，选中项需有明确高亮反馈（描边/底色/文本色遵循 `c1` 规范反馈）。
   - 二级分类在保存时必须归属到当前选中的一级分类；未选中一级分类时不可提交二级分类。
...
**分类管理闭环（本次补充重点）**
 ...
 - 图标规范（新增）：
   ...
   - 分类列表、抽屉表单、图标选择器中的所有分类图标容器，均需复用 `c1` 规范：圆形浅色底、居中 Lucide 线性图标、统一删除/选中反馈。
- 停用规则：
...
## 6.2 分类管理闭环验收标准（新增）
 ...
 - [ ] 分类管理页标题“分类管理”在头部居中展示，顶部“支出 / 收入”切换控件水平居中。
 - [ ] 预设分类与自定义分类卡片结构一致，均展示二级分类摘要与明细；仅状态字段样式不同。
 - [ ] 新增分类抽屉内包含居中的 `typeTabsWrap` 组件，可单选“支出 / 收入”。
 - [ ] 新增分类抽屉中的 `drawerTypeTabs` 与页面 `typeTabsWrap` 视觉样式一致（区块色、选中态背景、字体颜色一致）。
 - [ ] 一级分类名称下方仅展示一级分类图标项并支持点击选中，不展示关闭叉按钮与 `iconAddRow`/回车添加区块。
 - [ ] 一级分类图标选中后具有明确选中反馈，且二级分类保存时归属到该选中的一级分类。
 - [ ] 分类列表、抽屉表单、图标选择器中的分类图标样式保持一致，均为圆形彩色背景的 `c1` 规范。
